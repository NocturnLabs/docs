...
```bash
# From within the opencode-autocode directory
cargo install --path .
```
...
git clone https://github.com/NocturnLabs/opencode-autocode
...
- `priority_order`: List of tool names (e.g., `["osgrep", "chat-history"]`).
- `prefer_osgrep`: Use `osgrep` over `grep` for smarter code search (`true`).
...
_Note: Regression checks require a real `feature_list.json` to exist, but won't modify files._

```bash
$ opencode-autocode regression-check --verbose
🔍 Running regression check on feature_list.json...
Checking: User Authentication
  ✓ PASS
Checking: Database Connection
  ✓ PASS
Checking: API Rate Limiting
  ○ MANUAL (no verification_command)
════════════════════════════════════════
...
──────────────────────────────────────────────────
  ...
  🌐 Full-Stack Web App (web-app-fullstack)
